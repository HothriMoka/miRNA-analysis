...
# Mouse-specific version without hardcoded blacklist paths
...
# Run plotHeatmap for signal visualization over RNA types
echo "Running plotHeatmap..."
plotHeatmap -m "$matrix_file" \
    -out "$output_dir/${output_prefix}_bed_density_heatmap.png" \
    --colorMap Blues \
    --legendLocation upper-left \
    --startLabel "5'" \
    --endLabel "3'" \
    --plotTitle "Signal Visualization in RNA Types" \
    --regionsLabel "${regions_label[@]}" \
    --heatmapHeight 12 \
    --heatmapWidth 6 \
    --dpi 300 || {
        echo "Error: plotHeatmap failed for PNG output."
        exit 1
    }

# Generate SVG version
echo "Running plotHeatmap for SVG..."
plotHeatmap -m "$matrix_file" \
    -out "$output_dir/${output_prefix}_bed_density_heatmap.svg" \
    --colorMap Blues \
    --legendLocation upper-left \
    --startLabel "5'" \
    --endLabel "3'" \
    ...
    --heatmapHeight 12 \
    --heatmapWidth 6 || {
        echo "Error: plotHeatmap failed for SVG output."
        exit 1
    }

echo "✓ Density heatmap generated:"
echo "  PNG: $output_dir/${output_prefix}_bed_density_heatmap.png"
echo "  SVG: $output_dir/${output_prefix}_bed_density_heatmap.svg"

# Clean up processed BED files (keep matrix for potential reuse)
for bed_file in "${processed_bed_files[@]}"; do
    rm -f "$bed_file"
done

echo "✓ Density plot generation complete!"
