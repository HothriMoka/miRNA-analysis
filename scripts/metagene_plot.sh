...
for i in "${!bed_files[@]}"; do
    ...
    output_bed="$output_dir/$(basename "${bed_file%.*}")_processed_meta.bed"
...
plotProfile -m "$matrix_file" \
    ...
    --startLabel "TSS (-1kb)" \
    --endLabel "TES (+1kb)" \
    --plotWidth 12 --plotHeight 8 --dpi 300 \
    --yAxisLabel "Mean Signal" \
    --plotTitle "Signal Visualization in Meta Gene Regions" \
    --plotType lines \
    --perGroup \
...
echo "✓ Meta-gene profile generated:"
echo "  PNG: $output_dir/${output_prefix}_bed_stacked_profile_meta_gene.png"
echo "  SVG: $output_dir/${output_prefix}_bed_stacked_profile_meta_gene.svg"
...
echo "✓ Meta-gene plot generation complete!"
